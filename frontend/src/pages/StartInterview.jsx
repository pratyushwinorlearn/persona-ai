...
export default function StartInterview({ onStart }) {
  ...
  useEffect(() => {
    ...
    // Kill any lingering ScrollTriggers from previous renders
    ...
    const ctxGSAP = gsap.context(() => {
      ...
      const onMouseMove = (e) => {
        if (spotlight) {
          gsap.to(spotlight, { x: e.clientX - 300, y: e.clientY - 300, duration: 0.8, ease: "power2.out" });
        }
      };
      ...
      // ── STICKY NAV BG ──────────────────────────────────────────
      ScrollTrigger.create({
        trigger: scroller, scroller, start: "top top", end: "+=600", scrub: true,
        onUpdate: self => {
      ...
      gsap.utils.toArray(".sec-hl").forEach(el => {
        ...
        gsap.fromTo(el.querySelectorAll(".sc"),
          ...
          { y: 0, opacity: 1, rotationX: 0, filter: "blur(0px)", stagger: { each: 0.018, from: "start" }, duration: 0.55, ease: "power3.out",
      ...
      gsap.utils.toArray(".bg-grid").forEach(grid => {
        grid.querySelectorAll(".bc").forEach((card, i) => {
          gsap.fromTo(card,
      ...
      gsap.utils.toArray(".bc").forEach(card => {
        const onMove = (e) => {
          ...
          if (sp) gsap.to(sp, { opacity: 1, x: e.clientX - r.left - 150, y: e.clientY - r.top - 150, duration: 0.3, ease: "power2.out" });
        };
        const onLeave = () => {
          gsap.to(card, { rotationY: 0, rotationX: 0, duration: 0.6, ease: "elastic.out(1, 0.5)" });
          const sp = card.querySelector(".card-spotlight");
          if (sp) gsap.to(sp, { opacity: 0, duration: 0.3 });
        };
      ...
      gsap.utils.toArray(".sec-doodle").forEach((el, i) => {
        gsap.fromTo(el, { opacity: 0, scale: 0.7, rotation: -15 },
      ...
      gsap.fromTo(".pdbar", { scaleY: 0, transformOrigin: "bottom" },
        { scaleY: 1, stagger: 0.07, duration: 0.8, ease: "elastic.out(1.2, 0.5)",
          scrollTrigger: { trigger: ".pd-wrap", scroller, start: "top 85%", toggleActions: "play none none none" } });
      gsap.fromTo(".wbar", { scaleY: 0, opacity: 0, transformOrigin: "center" },
      ...
      gsap.fromTo(".rcat", { x: -60, opacity: 0, rotationY: -25 },
        { x: 0, opacity: 1, rotationY: 0, stagger: 0.1, duration: 0.7, ease: "power3.out",
          scrollTrigger: { trigger: ".rcat-wrap", scroller, start: "top 82%", end: "top 36%", scrub: 0.8 } });

      gsap.utils.toArray(".rcat").forEach((cat, ci) => {
        gsap.fromTo(cat.querySelectorAll(".rcri"), { scale: 0.5, opacity: 0, y: 20 },
          { scale: 1, opacity: 1, y: 0, stagger: 0.06, duration: 0.5, ease: "back.out(2.5)",
            scrollTrigger: { trigger: cat, scroller, start: "top 78%", toggleActions: "play none none none" }, delay: ci * 0.1 });
      });

      gsap.fromTo(".rbanner", { scale: 0.88, opacity: 0, y: 30, clipPath: "inset(0% 100% 0% 0%)" },
        { scale: 1, opacity: 1, y: 0, clipPath: "inset(0% 0% 0% 0%)",
          scrollTrigger: { trigger: ".rbanner", scroller, start: "top 87%", end: "top 50%", scrub: 0.65 } });
      gsap.fromTo(".start-hl", { y: 85, opacity: 0, skewX: -4 },
        { y: 0, opacity: 1, skewX: 0,
          scrollTrigger: { trigger: ".start-hl", scroller, start: "top 86%", end: "top 40%", scrub: 0.9 } });
      gsap.fromTo(".form-card", { x: 75, opacity: 0, rotationY: 15, transformPerspective: 800 },
      ...
      gsap.utils.toArray(".sfi").forEach((el, i) => {
        gsap.fromTo(el, { y: 48, opacity: 0, x: -20 },
          { y: 0, opacity: 1, x: 0, duration: 0.6, ease: "power2.out",
            scrollTrigger: { trigger: ".sfi-wrap", scroller, start: "top 83%", toggleActions: "play none none none" }, delay: i * 0.15 });
      });
      ...
      gsap.fromTo(".mq-wrap", { opacity: 0, y: 20 },
        { opacity: 1, y: 0, scrollTrigger: { trigger: ".mq-wrap", scroller, start: "top 86%", toggleActions: "play none none none" } });
      ...
      gsap.utils.toArray(".count-num").forEach(el => {
        ...
        ScrollTrigger.create({
          ...
          onEnter: () => {
            ...
            gsap.fromTo(proxy, { val: 0 }, { val: target, duration: 2.2, ease: "power3.out",
              onUpdate: () => { el.textContent = isFloat ? proxy.val.toFixed(1) : Math.round(proxy.val) + suffix; } });
          },
        });
      });

      gsap.to("#noise", { backgroundPosition: "100% 100%", ease: "none",
        scrollTrigger: { trigger: scroller, scroller, start: "top top", end: "bottom bottom", scrub: true } });
      ...
      gsap.utils.toArray(".vert-label").forEach(el => {
        gsap.fromTo(el, { opacity: 0, x: el.classList.contains("vert-right") ? 20 : -20 },
      ...
      if (footerLogo) {
        ...
        gsap.fromTo(footerLogo.querySelectorAll("span"), { y: 40, opacity: 0, rotationX: -50 },
          { y: 0, opacity: 1, rotationX: 0, stagger: 0.06, duration: 0.5, ease: "back.out(1.8)",
            scrollTrigger: { trigger: "#footer", scroller, start: "top 88%", toggleActions: "play none none none" } });
      }

      gsap.fromTo("#footer", { opacity: 0, y: 40 },
        { opacity: 1, y: 0, scrollTrigger: { trigger: "#footer", scroller, start: "top 90%", end: "top 60%", scrub: 0.6 } });
      gsap.fromTo(".stat-block", { y: 50, opacity: 0, scale: 0.85 },
        { y: 0, opacity: 1, scale: 1, stagger: { each: 0.12, from: "start" }, duration: 0.65, ease: "back.out(1.8)",
          scrollTrigger: { trigger: ".stats-strip", scroller, start: "top 86%", toggleActions: "play none none none" } });

      ScrollTrigger.create({
        trigger: ".mq-wrap", scroller, start: "top bottom", end: "bottom top", scrub: true,
        onUpdate: self => {
          const speed = 32 - self.getVelocity() / 400;
          gsap.to(".mq", { duration: 0.5, "--mq-duration": `${Math.max(8, Math.min(80, speed))}s` });
        },
      });
      ...
      secRefs.current.forEach((sec, i) => {
        ...
        gsap.fromTo(sec, { background: "transparent" },
          { background: colors[i] || "transparent",
            scrollTrigger: { trigger: sec, scroller, start: "top 60%", end: "bottom 60%", scrub: 1 } });
        const wm = sec.querySelector(".sec-watermark");
        if (wm) gsap.fromTo(wm, { x: 80, opacity: 0 },
          { x: 0, opacity: 1, scrollTrigger: { trigger: sec, scroller, start: "top 70%", end: "top 20%", scrub: 1 } });
      });

      gsap.fromTo(".chip", { scale: 0, opacity: 0, rotation: -10 },
        { scale: 1, opacity: 1, rotation: 0, stagger: { each: 0.04, from: "random" }, duration: 0.45, ease: "back.out(2.2)",
          scrollTrigger: { trigger: ".chip-row", scroller, start: "top 85%", toggleActions: "play none none none" } });
      gsap.fromTo(".vst", { y: 30, opacity: 0, scale: 0.8 },
        { y: 0, opacity: 1, scale: 1, stagger: 0.1, duration: 0.5, ease: "back.out(1.7)",
          scrollTrigger: { trigger: ".vstats", scroller, start: "top 88%", toggleActions: "play none none none" } });

      ScrollTrigger.create({
        trigger: ".form-card", scroller, start: "top 70%", once: true,
        onEnter: () => {
          document.querySelectorAll(".field input, .field select").forEach((field, i) => {
            setTimeout(() => {
              gsap.fromTo(field,
                { boxShadow: "0 0 0 0px rgba(125,249,194,0)", borderColor: "rgba(255,255,255,0.07)" },
                { boxShadow: "0 0 0 4px rgba(125,249,194,0.08)", borderColor: "rgba(125,249,194,0.35)", duration: 0.4, yoyo: true, repeat: 1,
                  onComplete: () => gsap.set(field, { boxShadow: "", borderColor: "" }) });
            }, i * 180);
          });
        },
      });

      gsap.fromTo(".fquote", { clipPath: "inset(0% 100% 0% 0%)", opacity: 0 },
        { clipPath: "inset(0% 0% 0% 0%)", opacity: 1, duration: 1.2, ease: "power2.inOut",
          scrollTrigger: { trigger: ".fquote", scroller, start: "top 85%", toggleActions: "play none none none" } });
      ScrollTrigger.create({
        trigger: ".rbanner", scroller, start: "top 80%", once: true,
        onEnter: () => gsap.fromTo(".rb-n",
          { scale: 3, opacity: 0, rotation: -90, filter: "blur(20px)" },
          { scale: 1, opacity: 1, rotation: 0, filter: "blur(0px)", duration: 1.1, ease: "power4.out" }),
      });
  ...
  const scrollTo = i => {
    ...
    if (el && wrapRef.current) wrapRef.current.scrollTo({ top: el.offsetTop - 52, behavior: "smooth" });
  };

  async function handleStart() {
    if (!isAuthenticated) { setAuthMode("login"); return; }
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      if (showOtpInput) {
        ...
        setIsAuthenticated(true); setAuthMode(null); setShowOtpInput(false);
        return;
      }
      ...
      if (!res.ok) {
        ...
        if (authMode === "login") {
          localStorage.setItem("token", data.token);
          setIsAuthenticated(true); setAuthMode(null);
        } else {
  ...
  const handleOpenHistory = async () => {
    setShowHistory(true); setHistoryLoading(true); setSelectedInterview(null);
    try {
      const token = localStorage.getItem("token");
  ...
  const handleHistoryClick = async (id) => {
    setDetailsLoading(true); setSelectedInterview({ id });
    try {
      const token = localStorage.getItem("token");
      const res   = await fetch(`https://persona-ai-production-ac95.up.railway.app/api/auth/history/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) { const data = await res.json(); setSelectedInterview(data); }
      else setSelectedInterview(null);
    } catch (e) { console.error(e); setSelectedInterview(null); }
    finally { setDetailsLoading(false); }
  };

  const handleSignOut = () => { localStorage.removeItem("token"); setIsAuthenticated(false); };

  const closeAuth = () => { setAuthMode(null); setShowOtpInput(false); };
      ...
      <div id="main-wrap" ref={wrapRef}>
              ...
              <form onSubmit={handleAuthSubmit}>
                ...
                <button type="submit" className="btn-go" disabled={authLoading}>
                  <div className="btn-in">
                    {authLoading ? <><div className="spin" />Processing...</> : showOtpInput ? "Verify Code →" : authMode === "login" ? "Sign In →" : "Send Code →"}
                  </div>
                </button>
              </form>
              {!showOtpInput && (
        ...
        <section className="sec" ref={el => secRefs.current[0] = el}>
          ...
          <div className="bg-grid">
            <div className="bc" style={{ gridColumn:"span 8", gridRow:"span 2", minHeight:460 }}>
              ...
              <DoodleCorner style={{ position:"absolute", top:16, left:16, opacity:0.6, pointerEvents:"none" }} />
            ...
            <div className="start-r">
              <div className="bc form-card" ref={formRef}>
                ...
                <div className="field field-full">
                  ...
                  <input placeholder="e.g. Backend Developer, Product Manager..."
        ...
        <footer id="footer">
          <div className="ft-doodle-row">
            <DoodleWave /><DoodleDots /><DoodleCross size={28} /><DoodleHex />
          </div>
...
.card-spotlight {
  ...
  will-change:transform,opacity; transform:translate(-150px,-150px);
}
...
.stat-block::after { content:''; position:absolute; inset:0; background:linear-gradient(135deg,rgba(125,249,194,0.04),transparent 60%); opacity:0; transition:opacity 0.3s; }
...
#hero-container { position:relative; width:100vw; height:100vh; background:#000; overflow:hidden; }
#hero-canvas { display:block; width:100%; height:100vh; }
...
.chip { font-size:11px; font-weight:600; color:var(--m); background:rgba(255,255,255,0.04); border:1px solid var(--bdr); padding:6px 13px; border-radius:40px; transition:all 0.25s; cursor:pointer; will-change:transform,opacity; }
...
.sbfill { height:100%; background:linear-gradient(90deg, var(--a2), var(--a), var(--a2)); background-size:200% 100%; border-radius:2px; width:0%; }
...
.auth-overlay { position:fixed; inset:0; z-index:99990; display:flex; align-items:center; justify-content:center; }
.auth-backdrop { position:absolute; inset:0; background:rgba(0,0,0,0.7); backdrop-filter:blur(12px); cursor:auto; }
.auth-card { position:relative; z-index:2; width:100%; max-width:420px; animation:modalPop 0.5s cubic-bezier(0.16,1,0.3,1) forwards; background:rgba(13,13,13,0.85); backdrop-filter:blur(20px); }
...
.auth-close { position:absolute; top:20px; right:20px; background:none; border:none; color:var(--d); cursor:pointer; font-size:18px; font-family:monospace; transition:color 0.2s; z-index:10; }
.auth-close:hover { color:var(--w); }
.auth-switch { cursor:pointer; background:none; border:none; color:var(--a); font-size:13px; font-family:'Bricolage Grotesque',sans-serif; margin-top:20px; width:100%; text-align:center; transition:color 0.2s,opacity 0.2s; }
...
.history-item { display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.03); border:1px solid var(--bdr); padding:16px; border-radius:8px; transition:background 0.2s,border-color 0.2s; cursor:pointer; }
...
.history-score { font-family:'Bebas Neue',sans-serif; font-size:24px; letter-spacing:1px; padding:4px 12px; border-radius:4px; }
...
.history-back { background:none; border:1px solid var(--bdr); color:var(--d); padding:4px 10px; border-radius:4px; font-size:11px; cursor:pointer; font-family:'Bricolage Grotesque',sans-serif; text-transform:uppercase; transition:all 0.2s; letter-spacing:1px; }
